...
addSubSpecies <- function(infraList,Tree){
  ...
  #branchLength <- gsub("^.*Abies_magnifica.*?(\\d+\\.*\\d*).*", "\\1", exampleTree)
  branchLength <- as.numeric(gsub(paste0("^.*",species,".*?(\\d+\\.*\\d*).*"),"\\1", Tree))

# Creating String of subspecies that will replace "species"
  createSppBL <- function(infra){
    # HUGE ASSUMPTION HERE THAT BRANCH LENGTH FOR INFRASPECIES IS HALF THAT OF BRANCH LENGTHS BETWEEN GENERA
  ...
  FixedTree <- read.newick(text=gsub(paste0(species,":"),subSpeciesString,Tree))
...
abiesBound <- bind.relative(Spp1[genus=="Abies" & !(grepl("var.",species))])
...
result_california_treeV_abies<- phylo.maker(sp.list=SppVas1[genus=="Abies" & !(grepl("var.",species))]) 

# Working out how to nest subspecies within species
exampleTree <- write.tree(result_california_treeV_abies$scenario.3)


infraListAbies <- c("Abies_magnifica_var._magnifica","Abies_magnifica_var._shastensis")

obj <- addSubSpecies(infraListAbies,exampleTree)
plot(read.newick(text=obj[[4]]))

newAbiesTree <- addSubSpecies(infraListAbies,result_california_treeV_abies$scenario.3)
plot(newAbiesTree)
