...
data <- fread("Data/CAPR_Collections_Search_Results2019-06-26 (1).csv")
data[,genus:=word(`Taxon Name (Jepson)`,1,1)]

# Reading in CaPR species list
caprSpp <- caspio_get_table("tblSpeciesJepCAPR",login1,1)
TableCombinedSeedProSDZG = NULL
pagenumber=1

while (dim(caprSpp)[1]>0)
{
  pagenumber = pagenumber + 1
  TableCombinedSeedProSDZG <- rbind(TableCombinedSeedProSDZG,caprSpp)
  caprSpp <- caspio_get_table("tblSpeciesJepCAPR",login1,pagenumber)
}

caprSpp <- TableCombinedSeedProSDZG

# Reading in CNPS
cnps <- caspio_get_table("tblCNPSRanks",login1,1)
cnpsTEMP = NULL
pagenumber=1
...
{
  pagenumber = pagenumber + 1
  cnpsTEMP <- rbind(cnpsTEMP,cnps)
  cnps <- caspio_get_table("tblCNPSRanks",login1,pagenumber)
}
...
Spp1 <- Spp1[ grepl("1B",CRPR) | TJM1TJM2%in%c("TJM1","TJM2")]
...
Spp1 <- Spp1[,.(species,genus,family=familyTree,major_clade)]
...
GenNa1 <- merge(GenNa1,JepFam1, by="genus",all.x=T)
GenNa1[,Count.x:=NULL]
GenNa1[,Count.y:=NULL]
...
GenNa1[genus=="Aphyllon", Family:="Orobanchaceae"]
GenNa1[genus=="Mimetanthe", Family:="Phyrmaceae"]
...
SppVas1 <- Spp1[major_clade%in%c("GYMNOSPERMS","EUDICOTS","MONOCOTS","MAGNOLIDS","")]
...
# Writing data to the disc
  write.tree(result_california_speciesOnly$scenario.3,"Data/result_california_speciesOnly_20190829.tre")
  write.csv(result_california_speciesOnly$species.list,"Data/result_california_speciesOnly_20190829.csv")
...
# Reorganizing the pipeline
# spp <- fread("Data/tblSpeciesJepCAPR_2019-Jun-21_1946.csv")
# 
# spp[,name_minus_authors:=gsub(" ","_",name_minus_authors)]
# 
# # Sorting out which species have infra species
# spp[,binomial:=word(name_minus_authors,1,2,sep="_")]
# # Fixing situation with " x " (for crosses)
# 
# spp[,SppCount:=.N,by="binomial"]
# spp[,SppID:=1:.N,by="binomial"]
# spp[,infraSpp := toString(name_minus_authors[(grepl("var.",name_minus_authors,fixed=T)|grepl("subsp.",name_minus_authors,fixed=T))]),by="binomial"]
# 
# # Asking which collected species have infra species but also binomial level names
# spp[, Reassign:=ifelse((binomial==name_minus_authors & SppCount>1 ), "Yes","No")]
# spp[Reassign=="Yes",.(binomial,infraSpp)]
# spp[grepl("var.",infraSpp,fixed=T),InfraType:="var."]
# spp[grepl("subsp.",infraSpp,fixed=T),InfraType:="subsp."]
# spp[grepl("subsp.",infraSpp,fixed=T) & grepl("var.",infraSpp,fixed=T),InfraType:="both"]
# 
# # Fixing the one bad example
# spp[Reassign=="Yes"  & InfraType=="both", InfraType:="subsp."]
# 
# # Specifying the name on the phylogeny for each species (different than the Jep ID)
# spp[,nameOnPhylogeny:=ifelse(Reassign=="Yes",paste(name_minus_authors,InfraType,species,sep="_"),name_minus_authors)]
# 
# # Write out dataset for use with tree
# write.csv(spp,"Data/spp_withNameOnPhylogeny.csv")
...
write.csv(SppCopy,"Data/spp_withNameOnPhylogeny_20190626.csv")
...
write.tree(newTree, "Data/californiaTreewithInfraSpecies_20190829.tre")
