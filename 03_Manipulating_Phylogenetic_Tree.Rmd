...
sppOnlyTree <- read.tree("Data/result_california_speciesOnly_20190627.tre")
# California phylogeny with infraspecies created in 02 code
infraTree <- read.tree("Data/californiaTreewithInfraSpecies_20190626.tre")
# Accessions Data
...
summAcc <- AccwithSpp[,.(collectionTypes=toString(sort(unique(basisofRecord))),totalSeed=sum((organismQuantity),na.rm=T), totalSeedCollections=sum(1*(basisofRecord=="Seed")),totalColl=sum(1*(!is.na(eventID))), 
...
# Making a matrix
traitDT <- summAcc[!is.na(nameOnPhylogeny),.(nameOnPhylogeny,collectionTypes,totalSeed,totalSeedCollections,totalColl,family,genus)]
traitDT[is.na(totalSeedCollections),totalSeedCollections:=0]
# YesNo Variables
traitDT[,AnyCollYN:=ifelse(totalColl==0,0,1)]
traitDT[,SeedCollYN:=ifelse(totalSeedCollections==0,0,1)]
...
saveRDS(MatchDataObj, "MatchDataObj.rds")
